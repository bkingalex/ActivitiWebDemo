...
<!--显示表单内容-->
<div id=MainArea>
    <form action="/templateAction_update.do" method="post" enctype="multipart/form-data">
        <%--修改模板id'--%>
        ...
        <div class="ItemBlockBorder">
            <div class="ItemBlock">
                <table cellpadding="0" cellspacing="0" class="mainForm">
                    <tr>
                        <td>模板名称</td>
                        <td><input type="TEXT" name="name" class="InputStyle" value="${template.name}"/>  *</td>
                    </tr>
                    <tr>
                        <td>所用流程</td>
                        <td><select class="SelectStyle" name="pdkey">
                            <c:if test="${pdList!=null || fn:length(pdList)>0}">
                                <c:forEach items="${pdList}" var="item">
                                    <c:if test="${item.key == template.pdkey}"><option value="${item.key}" selected="selected">${item.name}</option>
                                    </c:if>
                                    <c:if test="${item.key != template.pdkey}"><option value="${item.key}">${item.name}</option>
                                    </c:if>
                                </c:forEach>
                            </c:if>
                        </select> *
